...
class EditContractTimelinePdfPage extends StatefulWidget {
  @override
  _EditContractTimelinePdfPageState createState() =>
      _EditContractTimelinePdfPageState();
}

class _EditContractTimelinePdfPageState
    extends State<EditContractTimelinePdfPage> {
  String headerLogo = '';
  String footerLogo = '';
  ...
  final TextEditingController _afterMilestoneController =
      TextEditingController();
  ...
  String introText =
      'We are pleased to announce that we have successfully entered into a contract for the property. While significant milestones have been achieved, there is still work to be done to reach closing. Enclosed, please find a comprehensive timeline and contact details for all parties involved in this transaction.';
  String middleText =
      'Successfully closing on this property will require a focused effort and careful attention to a number of details. To help guide us through this process, there are specific dates that we should all be aware of. Please keep these important milestones in mind as we continue to move forward in this transaction.';
  String afterMilestoneText =
  ...
  void initState() {
    ...
    _milestoneControllers.addAll(
      milestones
          .map((m) => TextEditingController(text: m['buyer'] ?? ''))
          .toList(),
    );
    _milestoneNameControllers.addAll(
      milestones
          .map((m) => TextEditingController(text: m['milestone'] ?? ''))
          .toList(),
    );
  }
  ...
  void _initializeEmptyContactDetails() {
    setState(() {
      contactDetails = List.generate(
        4,
        (_) => {'name': '', 'phone': '', 'email': '', 'company': ''},
      );
  ...
  void dispose() {
    ...
    for (var c in _milestoneControllers) {
      c.dispose();
    }
    for (var c in _milestoneNameControllers) {
      c.dispose();
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          footerLogo =
              'https://storage.googleapis.com/msgsndr/cyI1tRyaF0oYq5jaPVOP/media/67ec33c8e519ed016331cbfd.png';
  ...
  Future<void> _loadImagesFromApi() async {
    ...
    try {
      ...
      if (response['success'] == true &&
        ...
        Future<Uint8List?> _getImageBytes(dynamic imageData) async {
          ...
          try {
            ...
            // If not base64, try as URL
                ...
                Uri.tryParse(imageData)?.hasAbsolutePath == true) {
        ...
        // Load header image
        if (imageData['headerImage'] != null &&
            imageData['headerImage'].toString().isNotEmpty) {
        ...
        // Load footer image
        if (imageData['footerImage'] != null &&
            imageData['footerImage'].toString().isNotEmpty) {
  ...
  void updatePropertyAddress(String newAddress) {
    setState(() {
      ...
      introText =
          'We are pleased to announce that we have successfully entered into a contract for the property located at $newAddress. While significant milestones have been achieved, there is still work to be done to reach closing.';
    });
  }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          // Update controllers
          for (int i = 0; i < 4; i++) {
            contactControllers[i]['company']?.text =
                contactDetails[i]['company'] ?? '';
            contactControllers[i]['name']?.text =
                contactDetails[i]['name'] ?? '';
            contactControllers[i]['phone']?.text =
                contactDetails[i]['phone'] ?? '';
            contactControllers[i]['email']?.text =
                contactDetails[i]['email'] ?? '';
          }
        });
      }
    } catch (error) {
  ...
  Future<void> generatePDF() async {
    try {
              ...
              children: [
                ...
                pw.Row(
                  children: [
                    ...
                    pw.Text(
                      header['propertyAddress'] ?? 'Not specified',
                      style: defaultStyle,
                    ),
                  ],
                ),
                  ...
                  children: [
                    ...
                    pw.TableRow(
                      decoration: pw.BoxDecoration(
                        color: PdfColor.fromInt(0xFFD9E6F2),
                      ),
                      children: [
                        pw.Padding(
      ...
      final fileName =
          '${propertyAddress.replaceAll(' ', '_')}_$formattedDate.pdf';
  ...
  Future<void> _selectDate(BuildContext context, int index) async {
    ...
    if (picked != null) {
      setState(() {
        milestones[index]['buyer'] =
            '${picked.month}/${picked.day}/${picked.year}';
        _milestoneControllers[index].text =
            '${picked.month}/${picked.day}/${picked.year}';
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final double headerImageWidth =
        isMobile ? MediaQuery.of(context).size.width * 0.5 : 450;
    final double headerImageHeight = 180;
    final double footerImageWidth =
        isMobile ? MediaQuery.of(context).size.width * 0.4 : 300;
                                ...
                                child:
                                    _isLoadingImages
                                        ...
                                        ? Image.memory(
                                          _headerImageBytes!,
                                          fit: BoxFit.contain,
                                        ) // Show API image first
                                        : headerLogo.isNotEmpty
                                        ? Image.network(
                                          headerLogo,
                                          fit: BoxFit.contain,
                                        ) // Fallback to GHL logo
                                        : Column(
                                          mainAxisAlignment:
                                  ...
                                  _isLoadingFooterImage
                                      ...
                                      ? Image.memory(
                                        ...
                                        errorBuilder: (
                                          ...
                                          print(
                                            'Error displaying memory image: $error',
                                          );
                      ...
                      Flexible(
                        child: Column(
                          ...
                          children: [
                            ...
                            SizedBox(height: 4),
                            Text('786-486-6082', style: footerTextStyle),
                          ],
                        ),
                      ),
                    ],
