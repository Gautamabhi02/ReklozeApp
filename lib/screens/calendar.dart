...
class CalendarNotifier extends StateNotifier<List<ContractDateNote>> {
  ...
  // Date mapping similar to EditContractTimelinePdfPage
  ...
  List<String> get contractNames =>
      dropdownOptions.map((e) => e['label'] ?? '').toList();
}

// ---------------------- PROVIDERS ----------------------
final calendarProvider =
    StateNotifierProvider<CalendarNotifier, List<ContractDateNote>>((ref) {
      return CalendarNotifier()..fetchOpportunityValue();
    });
...
class _CalendarPageState extends ConsumerState<CalendarPage> {
  ...
  final Map<String, Color> _noteColors = {
    'Effective Contract Date': Colors.blue.shade400,
    'Initial escrow deposit Due Date': Colors.purple.shade400,
    'Loan Application Due Date': Colors.green.shade400,
    'Additional Escrow Deposit Due Date': Colors.orange.shade400,
    'Inspection Period Ends': Colors.teal.shade400,
    'Loan Approval Period Ends': Colors.indigo.shade400,
    'Title Evidence Due Date': Colors.amber.shade600,
  ...
  IconData _getNoteIcon(String note) {
    switch (note) {
      case 'Effective Contract Date':
        return Icons.star;
      case 'Initial escrow deposit Due Date':
        return Icons.account_balance;
      case 'Loan Application Due Date':
        return Icons.monetization_on;
      case 'Additional Escrow Deposit Due Date':
        return Icons.attach_money;
      case 'Inspection Period Ends':
        return Icons.home_work;
      case 'Loan Approval Period Ends':
        return Icons.assignment_turned_in;
      case 'Title Evidence Due Date':
                      ...
                      contractList
                          .map(
                            (name) => DropdownMenuItem(
                              value:
                                  notifier.dropdownOptions.firstWhere(
                                    (e) => e['label'] == name,
                                  )['value'],
                              child: Text(
                                name,
